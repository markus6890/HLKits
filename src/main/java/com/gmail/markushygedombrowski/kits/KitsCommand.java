...
public class KitsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String alias, String[] args) {
        ...
        if(alias.equalsIgnoreCase("hlkitlist")) {
            listKits(cmdPlayer);
            return true;
        }
        if (args.length == 0) {
            ...
            cmdPlayer.sendMessage("§8[§6§lKits§8]§7 /hlreplacekit <kit> <optional money>");
            return true;
        }
        String kit = args[0];
        String playerName;
    ...
    public  void listKits(Player player) {
        player.sendMessage("§8[§6§lKits§8]§7 Kits:");
        player.sendMessage(kitsManager.getList());
